...
```{r loadin}
metadata <- read_csv(here(edna_combined, '20260319_peco-combined_metadata.csv')) #%>%
#  mutate(region_name = case_when(site_code == 'NC_WP' ~ 'CA_NC',
#                                 TRUE ~ region_name)) %>%
#  mutate(region_name_label = case_when(region_name == 'CA_NC' ~ 'Bodega Bay, California',
#                                       region_name_label == 'San Fransisco Bay, California' ~ 'San Francisco Bay, California',
#                                       TRUE ~ region_name_label))


taxonomy <- read_csv(here(edna_combined, '20251113_peco-combined_12s_taxonomy.csv'))
...
motumatrix_metadata_nolab_2021 <- motumatrix_metadata_2021 %>%
  ...
  relocate(
           ctrl_lab_fish, .before = reads_adjust_lab) %>%
  relocate(sample_id, motu, found_taxa, common_name, rank, year) %>%
  select(!c(is_fish_lab))
```
...
motumatrix_metadata_nofield_2021 <- motumatrix_metadata_nolab_2021 %>%
  ...
  ## made histogram, assigned in first "real" gap at a binwidth = 5; above 50
  ## adjusted for miseq
  ...
  mutate(flag = case_when(sample_richness > 55 ~ "richness suspiciously high",
  ...
  relocate(in_ctrl_field)

#summarize the richness in every sample
sample_summaries<- motumatrix_metadata_nofield_2021 %>%
  select(year, 
         region_name, 
         site_code, 
         sample_id, 
...
ggplot(sample_summaries) +
  geom_histogram(aes(x = richness_sample_avs_in_fieldblank),
                 binwidth = 5)

# don't let these affect the semiquantitative
motumatrix_metadata_2021_qcd <- motumatrix_metadata_nofield_2021 %>%
  filter(is.na(flag))

#but add in at the end
motumatrix2021_fish_flagged <- motumatrix_metadata_nofield_2021 %>%
  filter(!is.na(flag))
...
motumatrix_metadata_2022 <- asvtable_2022 %>%
  ...
  mutate(seq_method = 'nextseq-2022')
```
...
pcr_ctrl_2022 <- motumatrix_metadata_2022 %>%
  ...
  select(sample_id, seq_method, asv, reads_raw, found_taxa) %>%
  mutate(pcrasv = paste0(seq_method, '_', asv)) %>%
  select(pcrasv, seq_method, reads_raw, found_taxa) %>%
  rename(reads_control = reads_raw)
...
motumatrix_metadata_2023 <- asvtable_2023 %>%
  ...
  left_join(metadata) %>%
    # remove columns that don't matter this year
  janitor::remove_empty() %>%
  mutate(seq_method = 'nextseq-2023')
...
miseq_2022_filtered <- motumatrix_metadata_2022_miseq %>%
  ...
  mutate(seq_method = paste0('miseq-', year)) %>%
  mutate(sample_id = paste0(sample_id, '_ms')) %>%
...
asv_all <- motumatrix_metadata_nofield_2022 %>% 
  ...
  janitor::remove_empty() %>%
  filter(!(sample_id %in% missing_samples$sample_id)) %>%
  bind_rows(miseq_2022_filtered, motumatrix_metadata_2021_qcd) %>%
  relocate(sample_id, motu, found_taxa, common_name, rank, year) %>%
...
fishonly_relabund <- fishonly_range %>%
  ...
  janitor::remove_empty() %>%
    mutate(sample_id = str_remove_all(sample_id, '_ms')) %>%
  mutate(remove = case_when(seq_method == 'miseq-2022' & !(sample_id %in% missing_samples$sample_id) ~ 'x')) %>%
  filter(is.na(remove)) %>%
  select(!remove)
...
fish_vstest %>%
  ggplot() +
  geom_histogram(aes(x = relabund_vst_taxa_round)) +
...
# pngs
png(file = here('output', 'controls', 'untransformed_large.png'),
...
png(file = here('output', 'controls', 'vst_large.png'),
    width = 800, height = 500)
pheatmap(assay(vsd_taxa), 
         fontsize = 5,
         cluster_cols = FALSE, cluster_rows = FALSE,
...
png(file = here('output', 'controls', 'wisconsin_large.png'),
    width = 800, height = 500)
pheatmap(wis_taxa, 
         fontsize = 5,
         cluster_cols = FALSE, cluster_rows = FALSE,
...
png(file = here('output', 'controls', 'wisconsin_large2.png'),
    width = 800, height = 500)
pheatmap(wis_taxa, 
         fontsize = 5,
         cluster_cols = FALSE, cluster_rows = FALSE,
...
ggsave(here('output', 'controls',  'unadjusted_pcoa.pdf'))
...
ggsave(here('output', 'controls',  'vst_pcoa.pdf'))
...
ggsave(here('output', 'controls',  'wisconsin_pcoa.pdf'))

```

### miseq vs nextseq vst
```{r mi and next}
fish_vstest %>%
  ggplot() +
...
fish_vstest %>%
  select(relabund_vst_taxa_round, n_vst_value, seq_method) %>%
  distinct() %>%
  ggplot() +
  geom_col(aes(x = relabund_vst_taxa_round,
               y = n_vst_value),
...
```{r direct comparison, eval=FALSE, include=FALSE}
fish_vstest2 <- fish_vstest %>%
  mutate(sample_id = str_remove_all(sample_id, '_ms')) %>%
  filter(year == 2022) %>%
  mutate(relabund_vst_taxa_round = round(relabund_vst_taxa, digits = 2)) %>%
  select(sample_id, seq_method, relabund_vst_taxa_round, 
         found_taxa, motu, asv, reads_raw) %>%
  distinct() %>%
  pivot_wider(values_from = c(relabund_vst_taxa_round, reads_raw),
              names_from = seq_method) %>%
  janitor::clean_names() %>%
  dplyr::rename(vst_nextseq = relabund_vst_taxa_round_nextseq_2022,
         vst_miseq = relabund_vst_taxa_round_miseq_2022)

fish_vstest2 %>%
  ggplot() +
  geom_point(aes(y = vst_nextseq,
                 x = vst_miseq),
             alpha = .5) +
  theme_bw()

ggsave(here('output', 'controls', 'vst_miseq-nextseq.png'))
```


```{r}
fish_vstest3 <- fish_vstest %>%
  mutate(sample_id = str_remove_all(sample_id, '_ms')) %>%
  mutate(remove = case_when(seq_method == 'miseq-2022' & !(sample_id %in% missing_samples$sample_id) ~ 'x')) %>%
  filter(is.na(remove)) %>%
  select(!remove)
...
#saveforcompariosn <- fish_vstest2 %>%
#  dplyr::rename(vst_miseq_all = vst_miseq)
  

fish_vstest4 <- fish_vstest2 %>% 
  select(!c(reads_raw_nextseq_2022, reads_raw_miseq_2022)) %>%
  left_join(saveforcompariosn) %>%
  mutate(diff_miseq = vst_miseq - vst_miseq_all) %>%
  mutate(diff_next = vst_nextseq - vst_miseq_all) %>%
  relocate(vst_miseq_all, diff_miseq, diff_next, .before = reads_raw_nextseq_2022) %>%
  group_by(found_taxa) %>%
  mutate(reads_raw_miseq = sum(reads_raw_miseq_2022, na.rm = TRUE),
         reads_raw_nextseq = sum(reads_raw_nextseq_2022, na.rm = TRUE)) %>%
  ungroup() %>%
  select(!c(motu,asv, reads_raw_nextseq_2022, reads_raw_miseq_2022)) %>%
  distinct()

fish_vstest4 %>%
  ggplot() +
  geom_point(aes(x = vst_miseq,
                 y = vst_miseq_all)) +
  theme_bw()

ggsave(here('output', 'controls', 'vst_miseqall-subset.png'))

fish_vstest4 %>%
  ggplot() +
  geom_histogram(aes(x = diff_miseq)) +
  theme_bw()

ggsave(here('output', 'controls', 'vst_miseq-difference-hist.png'))

fish_vstest4 %>%
  ggplot() +
  geom_histogram(aes(x = diff_next)) +
  theme_bw()

ggsave(here('output', 'controls', 'vst_miseq-nextseq-difference-hist.png'))
