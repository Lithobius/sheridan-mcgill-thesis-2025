...
# miseq for 2021 and 2022
# Miseq data
miseq2021 <- read_csv(here(edna_combined,
                           '20241022_peco-2021_12s_asvmatrix_complete.csv')) %>%
  #filter(sample_id %in% missing_samples$sample_id) %>%
  mutate(seq_method = paste0('miseq-', year))%>%
  select(!c(dna_extraction_date)) %>%
  mutate(site_code = case_when(site_code == "NC_F" ~ "NC_BL",
                               TRUE ~ site_code))

miseq2022 <- read_csv(here(edna_combined,
                           '20241022_peco-2022_12s_motumatrix_complete.csv')) %>%
  filter(sample_id %in% missing_samples$sample_id) %>%
  mutate(seq_method = paste0('miseq-', year)) %>%
  select(!c(dna_extraction_date))
...
miseq_all <- miseq2021 %>%
  bind_rows(miseq2022) %>%
  mutate(region_name = case_when(site_code %in% c("YB_FP", "YB_BV") ~ "OR_CB",
                                  site_code %in% c("NC_PM", "NC_PS",
                                                   "NC_RB", "NC_WP") ~ "CA_SF",
                                  TRUE ~ region_name)) %>%
  mutate(region_code = case_when(site_code %in% c('YB_FP', 'YB_BV') ~ 'OR_CB',
                                 TRUE ~ region_code)) %>%
  ...
  mutate(site_code = case_when(site_code == 'PP_J' ~ 'PP_JJ',
...
asv_all <- motumatrix_metadata_nofield_2022 %>% #asvs2021 %>%
  #full_join(asvs2022) %>%
