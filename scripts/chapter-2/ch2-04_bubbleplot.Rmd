...
npages<-length(unique(mifish_edna$year))
...
for(i in 1:npages){
  ...
  bubble_plot_loop_data <- mifish_edna %>%
...
bigdat_fish <-  read_csv(here('processeddata','bigdata',
               '20250626_obis-gbif_region-peco-bysite_unfiltered.csv')) %>%
  # taxonomic revision
...
bubble_plot_data_sp %>%
  ...
  theme(axis.text = element_text(size = 8, family = 'muli'),
