...
# 
#region_environment <- read_csv(here('rawdata', 'temp', 'erddap',
#                                    '20251212_sitesst_peco_summarystats.csv'))
...
edna_peco <- read_csv(here('processeddata', 'peco', '2021-24_edna',
  ...
  # remove sites that were only one year
  filter(!(site_code %in% c('PP_EB', 'PP_S', 'PP_O', 'PP_M', 'PP_K', 'PP_SI'))) %>%
  mutate(region_name_label = ifelse(region_name == 'BC_HG', 'Haida Gwaii, British Columbia', region_name_label))

edna_peco_sp <- edna_peco %>%
  select(found_taxa, common_name, relabund_vst_taxa, sample_id, site_code, location_name,
         year, region_name, region_name_label) %>%
  distinct()
...
sti_peco <- edna_peco %>%
  ...
  mutate(region_label = ifelse(region_name == 'BC_HG', 'Haida Gwaii, British Columbia', region_label))
...
ggplot()+
  ...
  theme(axis.text.x = element_blank(),
        ...
        # to add my own numbers after trimming
        strip.background = element_blank(),
        legend.position = 'none',
        panel.grid.minor = element_blank())


ggsave(here(plotout, paste0(datefield, 'sticti_bysite_facet-year_presentation-vers.pdf')),
        height = 14,
       width = 14,
       unit = 'in')
```
...
thermal affinity with environment
```{r}
cti_environment_site_year <- cti_peco %>%
  left_join(site_temps) %>%
  # region sst for the year
  group_by(region_name,year) %>%
  mutate(region_meansst = mean(sst_mean, na.rm = TRUE)) %>%
  ungroup() %>%
  # fill in NAs with last 5 mean
  group_by(region_name) %>%
  mutate(region_last5_meansst = mean(sst_mean, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(region_meansst = case_when(region_meansst == 'NaN' ~ region_last5_meansst,
                                    TRUE ~ region_meansst))
...
ggplot()+
  geom_point(data = sti_peco,
             aes(x=sti_value,
                 y= factor(site_code, levels = site_order2$site_code),
                 color = region_label,
                 alpha = relabund_vst_taxa),
             size = 2) +
  geom_point(data = cti_environment_site,
             aes(y = factor(site_code, levels = site_order2$site_code),
                 x = cti),
             shape = 23,
             size = 3,
             fill = 'red') +
  geom_line(data = cti_environment_site_year,
             aes(y = factor(site_code, levels = site_order2$site_code),
                 x = region_meansst,
                 group = 1),
            orientation = 'y') +
...
ggsave(here(plotout, paste0(datefield, 'sticti_bysite_facet-year-meansst_presentation-vers.pdf')),
       height = 14,
       width = 14,
       unit = 'in')
```
...
npages <- round(length(splist)/6, 0)
# order of plotting is by number of detections
# then plot_group splits them into pages
rank_species <- edna_peco %>%
  select(found_taxa, common_name) %>%
  distinct() %>%
  arrange(found_taxa) %>%
  mutate(position = 1:n()) %>%
  mutate(plot_group=as.numeric(cut(position, npages)))#separate into 12 groups
...
#  target_sp <- splist[i]
  ...
  sti_unprocessed %>%
    ...
    pivot_longer(cols = c('sst', 'sst_n'),
               ...
               values_to = 'sst') %>%
    filter(!is.na(sst)) %>%
    left_join(sti_target) %>%
    mutate(taxa_label = paste0(found_taxa, ',\n', common_name)) %>%
  ggplot() +
  geom_histogram(aes(x = sst)) +
    geom_vline(aes(xintercept = sti_value), 
               color = '#4D94BAFF') +
    geom_vline(aes(xintercept = temp_95),
                   color = '#C9785CFF') +
    geom_vline(aes(xintercept = temp_05), 
                 color = '#8EE5EE') +
  scale_x_continuous(limits = c(-5,40)) +
  facet_grid(taxa_label ~ ., 
             scales = 'free') +
  theme_bw() +
  theme(axis.title.y = element_blank(),
        panel.grid = element_blank(),
  ...
  ggsave(filename = here(plotout, 'splevel', 'grouped', 
                         paste0(datefield, 'thermalstats_page_', i, '.pdf')),
       device = 'pdf', height = 11, width = 8.5, units = 'in')
}
```
...
```{r}
splist <- c('Three-spined stickleback')




tss <- sti_peco %>%
  filter(found_taxa == 'Gasterosteus aculeatus') 

tss_env <- cti_environment_site_year %>%
  filter(site_code %in% tss$site_code)
...
ggsave(here(plotout, paste0(datefield, 'tss.pdf')),
       height = 10,
       width = 10,
       unit = 'in')
```
