...
plotout <- here('output', 'thermalaffinity')
datefield <- '20250927_'
...
htmlwidgets::saveWidget(widget = interactive2,
                        file = '20250927_ctsm_interactive.html')
```
...
```{r whichfish}
whichfish <- sti_peco %>%
  select(site_code, temp_95, sti_value, region_label, found_taxa) %>%
  distinct() %>% 
  left_join(cti_environment_site_distinct) %>%
  #filter(!region_last5_temp95 == 'NaN') %>%
  mutate(below_95_95 = ifelse(temp_95 > site_last5_temp95, 'x', NA_character_)) %>%
  mutate(below_75_95 = ifelse(temp_95 > site_last5_temp75, 'x', NA_character_)) %>%
  mutate(below_mean_95 = ifelse(temp_95 > site_last5_meansst, 'x', NA_character_)) %>%
  mutate(below_95_sti = ifelse(sti_value > site_last5_temp95, 'x', NA_character_)) %>%
  mutate(below_75_sti = ifelse(sti_value > site_last5_temp75, 'x', NA_character_)) %>%
  mutate(below_mean_sti = ifelse(sti_value > site_last5_meansst, 'x', NA_character_)) %>%
  mutate(safe = ifelse(is.na(below_mean_sti) & is.na(below_75_sti) & is.na(below_95_sti) & 
                         is.na(below_mean_95) & is.na(below_75_95) & is.na(below_95_95), 'x', NA_character_))


safefish <- whichfish %>%
  filter(safe == 'x') %>%
  select(region_label, found_taxa) %>%
  distinct()

verydangerfish <- whichfish %>%
  filter(is.na(safe)) %>%
  filter(below_mean_95 == 'x' | below_mean_sti == 'x') %>%
  select(found_taxa, region_label, below_mean_95, below_mean_sti) %>%
  distinct()

sortadangerfish <- whichfish %>%
  filter(is.na(safe)) %>%
  filter(below_75_95 == 'x' | below_75_sti == 'x') %>%
  select(found_taxa, region_label, below_75_95, below_75_sti) %>%
  distinct()

notverydangerfish <- whichfish %>%
  filter(is.na(safe)) %>%
  filter(below_95_sti == 'x' | below_95_95 == 'x') %>%
  select(found_taxa, region_label, below_95_95, below_95_sti) %>%
  distinct()
...
whichfish2 <- sti_peco %>%
  ...
  left_join(cti_environment_site_distinct) %>%
  #filter(!region_last5_temp95 == 'NaN') %>%
  mutate(dangerlevel = case_when(
                                 temp_95 < site_last5_meansst ~ '4 highest',
                                 temp_95 < site_last5_temp75 ~ '3 high',
                                 temp_95 < site_last5_temp95 ~ '3 uncomfortable',
                                 TRUE ~ '1 safe')) %>%
...
write_csv(whichfish2, here(dataedna, '20250713_fishatrisk.csv'))
#whichfish2 <- read_csv(here(dataedna, '20250713_fishatrisk.csv'))
```
