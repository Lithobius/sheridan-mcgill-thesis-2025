...
site_environment <- read_csv(here('rawdata', 'temp', 'erddap',
  ...
  #   mutate(region_label = case_when(site_code == 'NC_WP' ~ 'Bodega Bay, California',
  ...
  # distinct(site_code, year, .keep_all = TRUE)
...
fish_for_table <- sti_peco %>%
  select(site_code, region_lat_name, temp_95, sti_value, region_label, found_taxa, common_name) %>%
  distinct() %>% 
  left_join(cti_environment_site_distinct) %>%
  mutate(dangerlevel = case_when(temp_95 < site_last5_meansst ~ '4 highest',
                                 temp_95 < site_last5_temp75 ~ '3 high',
...
```{r}
bb_affinity <- affinity_cold %>%
  filter(region_name == 'CA_NC') %>%
  filter(year == 2024)
```
