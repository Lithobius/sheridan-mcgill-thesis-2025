...
test hg with tutorial 

```{r}
## fit a LMM with a random effect on the intercept
## i.e. thermal limits vary by Class
mod5 <- lmer(cti_value ~ sst_mean + (1 | site_code), data = hg_subset)
summary(mod5)["call"] ## check model formula
...
plot(prc$site_code)
screeplot(prc$site_code)

## plot random effects
## how much did thermal tolerance vary across classes?
## make dot and whisker plots of group means and CIs from the model
rand_effs <- broom.mixed::tidy(mod5, ## get tidy to provide random effects
                  effects = "ran_vals")

ggplot(data = rand_effs, 
       mapping = aes(x = level, y = estimate))+ 
  geom_pointrange(mapping = aes(ymin = estimate - 1.96 * std.error,
                                ymax = estimate + 1.96 * std.error), 
                  colour = "blue") + 
  geom_hline(yintercept = 0, lty = 2) + ## geom_hline b/c coord_flip() next
  coord_flip()


## plot fitted model over data
## first predict thermal tolerance for each row of data, at each level
pred_pop <- predict(mod5, re.form = NA) ## whole population level
pred_group <- predict(mod5) ## group level

## now plot
ggplot(data = hg_subset, 
       mapping = aes(x = sst_mean, y = cti_value, 
                     colour = site_code)) + 
  #geom_point(mapping = aes(y = thermal_limit)) + 
  geom_line(mapping = aes(y = pred_group)) + 
 # geom_line(mapping = aes(y = pred_pop), colour = "black") + 
  theme(legend.position = "bottom")
...
```{r}
test_subset <- cti_environment %>%
    filter(region_name == "CA_MB") 

## fit a LMM with a random effect on the intercept
## i.e. thermal limits vary by Class
mod5 <- lmer(cti_value ~ sst_mean + (1 | site_code), data = test_subset)
summary(mod5)["call"] ## check model formula
broom.mixed::tidy(mod5)
...
```
```{r}
regions_2 <- region_order %>%
  filter(!(region_name %in% c('CA_HB')))

region_list2 <- unique(regions_2$region_name)
...
for (i in 1:length(region_list)) {
  ...
  print(isSingular(region_model))
}
```

```{r}
hg_model <- lmer(cti_value ~ sst_mean + (1 | site_code), 
                       data = hg_subset)

hg_b_model <- blmer(cti_value ~ sst_mean + (1 | site_code), 
                       data = hg_subset)

rand_effs <- broom.mixed::tidy(hg_b_model, ## get tidy to provide random effects
                  effects = "ran_vals")
...
## plot fitted model over data
## first predict thermal tolerance for each row of data, at each level
pred_pop <- predict(hg_b_model, re.form = NA) ## whole population level
pred_group <- predict(hg_b_model) ## group level
