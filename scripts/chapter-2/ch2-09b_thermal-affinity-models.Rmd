...
# do i use this?
#w_sitenames <- edna_peco %>%
#  select(site_code, location_name, decimal_latitude, decimal_longitude,
#         region_name, region_label, region_code) %>%
#  distinct()
#write_csv(w_sitenames,here('site_names.csv'))
```
...
cti_environment <- cti_peco %>%
  ...
  mutate(region_mean = mean(sst_mean)) %>%
  ungroup() %>%
...
for (i in 1:length(region_list)) {
  ...
  region_model <- lme(cti_value ~ region_temp95,random=~1|site_code,
                      data=region_subset)
...
for (i in 1:length(region_list)) {
  ...
  region_model <- lme(cti_value ~ region_temp75,
                      random=~1|site_code,
                      data=region_subset)
