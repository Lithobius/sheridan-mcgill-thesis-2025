...
plotout <- here('output', 'chapter2', 'thermalaffinity', 'models')
datefield <- '20260223_'
...
regions <- read_csv(here(dataedna, '20251113_peco_region-name-plotting.csv'))[-1] %>%
  ...
  select(!c(region_code,region_code_label))
...
edna_peco_sp <- edna_peco %>%
  select(found_taxa, common_name, relabund_vst_taxa, sample_id, site_code, location_name,
         year, region_name, region_label) %>%
  distinct()
...
#                                    '20251113_mean-summer-sst_by-region.csv')) %>%
...
#                                   '20251113_summer-sst-summary-stats_by-site.csv')) 
site_environment <- read_csv(here('rawdata', 'temp', 'erddap',
...
for (i in 1:length(region_list)) {
  ...
  # random effects
  random_effect <- model_summary$varcor$site_code[1]
  # standard deviation of the random effect
  #stdev <- attr(model_summary$varcor$site_code, 'stddev')
  # residuals
  resid <- attr(model_summary$varcor, 'sc')^2
  ...
  ttable <- data.frame(model_summary$coefficients) %>%
    ...
    mutate(region_name = region_list[i],
           aic = model_summary$AICtab,
           random_variance = random_effect,
...
cti_95_out <- region_output %>%
  ...
  mutate(across(.cols = where(is.numeric),
                .fns = ~ round(., digits = 3))) %>%
  mutate(model = '95th percentile SST')
...
#model_summary$varcor$site_code[1]
#attr(model_summary$varcor$site_code, 'stddev')
# residuals?
#
model_summary$varcor
attr(model_summary$varcor, 'sc')
...
cti_75_out <- region_output %>%
  ...
  mutate(across(.cols = where(is.numeric),
                .fns = ~ round(., digits = 3))) %>%
  mutate(model = '75th percentile SST')
...
for (i in 1:length(region_list)) {
  ...
  ggsave(here(plotout, paste0(datefield, 'sti-sstmean_region-',
      ...
      # 
  ggsave(here(plotout, 'forpresentation', paste0(datefield, 'sti-sstmean-region-',
                                     region_list[i], '.png')),
         height = 2,
         width = 2,
         units = 'in')
...
region_model <- lmer(cti_value ~ beuti_year + (1 | site_code), 
 ...
 # theme(legend.position = 'none',
  ...
  ggsave(here(plotout, paste0(datefield, 'sti-upwelling_region-',
    ...
    ggsave(here(plotout, 'forpresentation', paste0(datefield, 'sti-upwelling_region-',
                                     region_list_upwell[i], '.png')),
         height = 2,
         width =2,
         units = 'in')
...
region_output %>%
  ...
  mutate(std_positive = intercept + std_error,
         std_negative = intercept - std_error,
         significant = ifelse(pr_t < .05, 'yes', 'no')) %>%
  #mutate(std_positive = intercept + sd,
  ##       std_negative = intercept - sd,
  #       significant = ifelse(pr_t < .05, 'yes', 'no')) %>%
  left_join(region_order)%>%
  mutate(region_label = factor(region_label, levels = region_order$region_label)) %>%
  ggplot(aes(x = intercept,
...
hg_site_list <- unique(hg_subset$site_code)

hg_output <- {}


for (i in 1:length(hg_site_list)) {
  print(hg_site_list[i])
  
  site_subset <- hg_subset %>%
    filter(site_code == hg_site_list[i]) #%>%
    #filter(year > 2021)
  
site_model <- lm(cti_value ~ sst_mean, 
                       data = site_subset)
...
hg_model <- lm(cti_value ~ sst_mean, data = hg_subset)
summary(hg_model)


hg_mean_subset <- hg_subset %>%
  group_by(site_code) %>%
  mutate(mean_cti = mean(cti_value),
         mean_sst_mean = mean(sst_mean)) %>%
  ungroup() %>%
  dplyr::select(site_code, mean_cti, mean_sst_mean) %>%
  distinct()

hg_mean_model <- lm(mean_cti ~ mean_sst_mean, data = hg_mean_subset)
summary(hg_mean_model)
plot(hg_mean_model)


hg_subset %>%
  ggplot() +
  geom_boxplot(aes(x = sst_mean,
                 y = cti_value,
                 colour = site_code)) +
  theme_bw()
...
site_cti_aov <- aov(cti_value ~ site_code, 
                    data = hg_subset)

summary(site_cti_aov)
report::report(site_cti_aov)


# anova: is there a difference in means between groups?
site_sst_aov <- aov(sst_mean ~ site_code, 
                    data = hg_subset)

summary(site_sst_aov)
report::report(site_sst_aov)

# tukey; which group?
library(multcomp)

site_sst_aov$model$site_code = as.factor(site_sst_aov$model$site_code)

site_sst_aov_posttest <- glht(site_sst_aov,
                                        linfct = mcp(site_code = "Tukey"))

summary(site_sst_aov_posttest)
plot(site_sst_aov_posttest)


ggstatsplot::ggbetweenstats(data = hg_subset,
                            x = site_code,
                            y = sst_mean,
                            type = 'parametric',
                            var.equal = TRUE,
...
```{r}
mb_subset <- cti_environment %>%
    filter(region_name == "CA_MB") %>%
  mutate(site_code_plot = str_remove_all(site_code, 'MB_'))

# mean  
mbmean_model <- lmer(cti_value ~ sst_mean + (1 | site_code), 
                       data = mb_subset)
  
# make a ggpredict plot object
# plot of lines per site 
mb_pred_pop <- predict(mbmean_model, re.form = NA) ## whole population level
mb_subset$pred_group <- predict(mbmean_model) ## group level

  
# subset sti values
mbsti_4plot <- sti_peco %>%
    left_join(cti_environment) %>%
    filter(region_name == "CA_MB") 


ggplot(data = mb_subset, 
       mapping = aes(x = sst_mean, y = cti_value)) + 
      # points for sti vs sst
    geom_point(data = mbsti_4plot,
               aes(y = sti_value,
                 x = sst_mean,
                 color = region_label,
                 size = relabund_vst_taxa),
  ...
  geom_line(aes(y = pred_group,
                ...
                linetype = site_code)) + 
  scale_color_manual(values = pal_2or) +
  geom_line(mapping = aes(y = mb_pred_pop), colour = "black") + 
  theme(legend.position = "bottom") +
  labs(x = 'Mean Summer SST', y = 'Thermal affinity',
       title = NULL)+
  facet_wrap('site_code') +
...
# pdf for figures
ggsave(here(plotout, paste0(datefield, 'sti-sstmean_morrobay-sitefacet.pdf')),
         height = 3,
         width = 5,
         units = 'in')

# png for presentation
ggsave(here(plotout, paste0(datefield, 'sti-sstmean_morrobay-sitefacet.png')),
         height = 3,
         width = 5,
         units = 'in')
...
# negatives
```{r}
nv_subset <- cti_environment %>%
    filter(region_name == "BC_NV")


nv_stis <- sti_peco %>%
  filter(region_name == "BC_NV") %>%
  left_join(nv_subset) %>%
  relocate(year, sst_mean, cti_value, relabund_vst_taxa)


nv_stis %>%
  filter(year == 2021) %>%
  #select(sst_mean, cti_value, year, site_code) %>%
  distinct() %>%
  ggplot() +
  geom_point(aes(x = sti_value,
                 y = relabund_vst_taxa,
                 colour = as.factor(year))) +
  theme_bw()

nv_stis %>%
  dplyr::select(sst_mean, cti_value, year, site_code) %>%
  distinct() %>%
  ggplot() +
...
# try model
# mean  
nvmean_model <- lmer(cti_value ~ sst_mean + (1 | site_code), 
                       data = nv_subset)
  
# make a ggpredict plot object
# plot of lines per site 
nv_pred_pop <- predict(nvmean_model, re.form = NA) ## whole population level
nv_subset$pred_group <- predict(nvmean_model) ## group level

  
# subset sti values
nvsti_4plot <- sti_peco %>%
    left_join(cti_environment) %>%
    filter(region_name == "BC_NV") 


ggplot(data = nv_subset, 
       mapping = aes(x = sst_mean, y = cti_value)) + 
      # points for sti vs sst
    geom_point(data = nvsti_4plot,
               aes(y = sti_value,
                 x = sst_mean,
                 color = region_label,
                 size = relabund_vst_taxa),
  ...
  geom_line(aes(y = pred_group,
                ...
                linetype = site_code)) + 
  scale_color_manual(values = pal_2or) +
  geom_line(mapping = aes(y = nv_pred_pop), colour = "black") + 
  theme(legend.position = "bottom") +
  labs(x = 'Mean Summer SST', y = 'Thermal affinity',
       title = NULL)+
  facet_wrap('site_code') +
...
# pdf for figures
ggsave(here(plotout, paste0(datefield, 'sti-sstmean_nvan-sitefacet.pdf')),
         height = 6,
         width = 6,
         units = 'in')

# png for presentation
ggsave(here(plotout, paste0(datefield, 'sti-sstmean_nvan-sitefacet.png')),
         height = 6,
         width = 6,
         units = 'in')
```
```{r}
sf_subset <- cti_environment %>%
    filter(region_name == "CA_SF")
...
# try model
# mean  
sfmean_model <- lmer(cti_value ~ sst_mean + (1 | site_code), 
                       data = sf_subset)
  
# make a ggpredict plot object
# plot of lines per site 
sf_pred_pop <- predict(sfmean_model, re.form = NA) ## whole population level
sf_subset$pred_group <- predict(sfmean_model) ## group level

  
# subset sti values
sfsti_4plot <- sti_peco %>%
    left_join(cti_environment) %>%
    filter(region_name == "CA_SF") 


ggplot(data = sf_subset, 
       mapping = aes(x = sst_mean, y = cti_value)) + 
      # points for sti vs sst
    geom_point(data = sfsti_4plot,
               aes(y = sti_value,
                 x = sst_mean,
                 color = region_label,
                 size = relabund_vst_taxa),
  ...
  geom_line(aes(y = pred_group,
                ...
                linetype = site_code)) + 
  scale_color_manual(values = pal_2or) +
  geom_line(mapping = aes(y = sf_pred_pop), colour = "black") + 
  theme(legend.position = "bottom") +
  labs(x = 'Mean Summer SST', y = 'Thermal affinity',
       title = NULL)+
  facet_wrap('site_code') +
...
# pdf for figures
ggsave(here(plotout, paste0(datefield, 'sti-sstmean_sanfran-sitefacet.pdf')),
         height = 6,
         width = 6,
         units = 'in')

# png for presentation
ggsave(here(plotout, paste0(datefield, 'sti-sstmean_sanfran-sitefacet.png')),
         height = 6,
         width = 6,
         units = 'in')
```
...
table_output <- cti_95_out %>%
  ...
  mutate(type = case_when(singular == TRUE ~ 'BLME',
                          singular == FALSE ~ 'LME')) %>%
  relocate(region_label, model, type, aic, random_variance, residual_variance) %>%
  dplyr::select(!c(region_name, avg_region_lat, region_lat_name)) %>%
  mutate(term = case_when(term == '(Intercept)' ~ "Intercept",
                          #term == '(Intercept)' ~ stringr::str_to_sentence(paste0(model, " intercept")),
