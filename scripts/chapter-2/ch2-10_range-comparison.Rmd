...
```{r big data}
bigdat_fish <- read_csv(here(bigdata,
...
bigdat_pecofish <- bigdat_fish %>%
  ...
  select(!(c(taxa_median, gbif_species, worms_aphiaid, rank))) %>%
  distinct()
...
bigdat_range <- bigdat_pecofish %>%
  select(species, bd_max, bd_min, bd_median) %>%
  distinct() 
...
ggsave(here(ranges, paste0(datefield, 'edna-obisgbif-allyears_species-range-lines.pdf')), 
...
for(i in 1:length(splist)){
  ...
  target_ranges <- bubble_plot_data_sp %>%
    ...
    select(bd_max, bd_min, lit_max, lit_min) %>%
    distinct()
