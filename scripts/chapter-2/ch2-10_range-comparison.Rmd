...
# save in
ranges <- here::here("output", 'chapter2', "rangecomparison")
...
fish_lit <- read_csv(here('rawdata', 'peco', '20250626_range-from-literature.csv')) %>%
  rename(lit_max = max_lat,
         lit_min = min_lat,
         species = found_taxa) %>%
  dplyr::select(!c(min_long, max_long)) %>%
  mutate(lit_max = case_when(species == 'Gasterosteus aculeatus' ~ 60,
                             TRUE ~ lit_max))
...
ggsave(here(ranges, paste0(datefield, 'edna-bigdata_species-range-lines_presentation.png')), 
...
ggsave(here(ranges, paste0(datefield, 'edna-literature_species-range-lines_presentation.png')), 
       height = 3500, width = 9000, units = 'px', 
       device = 'png')

```

## adding detectability to range comparison

```{r}
lit_prevalence <- prevalence_sp %>%
  dplyr::select(species, mean_prev_year) %>%
  distinct() %>%
  right_join(fish_order_lit)

range_prev <- lit_prevalence %>%
  arrange(overlap, abs_range) %>%
  mutate(fishorder = factor(common_name, levels = common_name)) %>%
  ggplot(aes(x = fishorder)) +
  #set up eDNA
  ...
  geom_segment(aes(y = lit_max, yend = lit_min, 
               ...
               col = pal[5],
               position = position_nudge(x = .5)) +
  geom_point(aes(x = common_name,
                 y = 32,
                 size = mean_prev_year/1.5,
                 alpha = mean_prev_year
                 )) +
  geom_point(data = region_order, aes(x = Inf, 
                                      y = region_order$avg_region_lat,
                                      color = region_order$region_label),
  ...
  scale_color_manual(values = pal_2or)+
  labs(x = NULL, y = NULL) +
  scale_y_continuous(limits = c(31.75,60))+
  # a little extra space for the points marking the sampling sites
  scale_x_discrete(expand = c(0,1.5)) +
  coord_cartesian(clip = 'off') +
  theme_pubclean()+
...
ggsave(here(ranges, paste0(datefield, 'edna-literature_species-range-lines-prevalence_presentation.png')), 
       height = 3500, width = 9500, units = 'px', 
       device = 'png')
...
ggsave(here(ranges, paste0(datefield, 'edna-literature_species-range-lines-prevalence_with-species.png')), 
       height = 4500, width = 9500, units = 'px', 
       device = 'png')
...
for(i in 1:length(splist)){
  ...
  ggsave(filename = here(ranges, 'splevel', paste0(datefield, 'large-bubble_', target_sp, '.png')),
         device = 'png', height = 3000, width = 1000, units = 'px')
}
```
...
suggestion was heatmap with values for concordance
could be like bubble plot but with color for status?
```{r heatmap}
edna_lit_heat <- mifish_edna %>%
  dplyr::select(found_taxa, common_name, site_code, region_label) %>%
  distinct() %>%
  mutate(species = found_taxa) %>%
  left_join(fish_order_lit)

fish_order_lit %>%
  ggplot() +
  geom_tile(aes(x = species, y = species, fill = overlap2))
