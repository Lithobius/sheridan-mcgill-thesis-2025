...
mifish_edna <- read_csv(here('processeddata', 'peco', '2021-24_edna',
                             '20251113_peco-combined_12s_asvmatrix_fish.csv')) %>%
  filter(rank == 'species') %>%
  select(found_taxa) %>%
  distinct() %>%
  arrange(found_taxa) %>%
...
# reload if needed
# sp_chunk_obis1 <- readRDS(here(obisdata, paste0(datefield, 'obis_spchunk1.RDS')))
# sp_chunk_obis2 <- readRDS(here(obisdata, paste0(datefield, 'obis_spchunk2.RDS')))
# sp_chunk_obis3 <- readRDS(here(obisdata, paste0(datefield, 'obis_spchunk3.RDS')))
# sp_chunk_obis4 <- readRDS(here(obisdata, paste0(datefield, 'obis_spchunk4.RDS')))
# sp_chunk_obis5 <- readRDS(here(obisdata, paste0(datefield 'obis_spchunk5.RDS')))
...
# load in combined data if needed
...
```{r anything missed}
# anything missed;
# there was an issue one time I ran the script
# so check if there's any missing, if so, uncomment and run here.
missed <- mifish_edna %>%
  filter(!(found_taxa %in% obis_sub$scientific_name)) 

#missed_obis <- obis_occur_sp(missed)

#missed_obis_sub <- missed_obis %>%
#  filter(!is.na(decimal_latitude)) %>%
#  #filter(!is.na(eventDate)) %>%
  #filter(marine == "TRUE") %>%
#  filter(occurrence_status %in% c('Present', 'present', 'presence', 
...
#                           TRUE ~ depth)) %>%
  # parse dates
#  separate_longer_delim(event_date, '/') %>%
#  mutate(event_date = str_remove_all(event_date, '-0700$|-0800$|-0300|-0400')) %>%
#  mutate(event_date = str_remove_all(event_date, '-08:00$|-03:00$|-04:00$|-07:00$|\\+00:00$|\\+02:00$|\\+12$|\\+09:00|\\+01:00|\\+10:00|\\+11:00|000\\+1100')) %>%
#    mutate(event_date = case_when(str_detect(event_date, "^[0-9]{2}$") ~ paste0(date_year, '-', event_date),
#                                TRUE ~ event_date)) %>%
#  mutate(lubri_date = lubridate::parse_date_time(event_date, c('ymd', 'ym', 'y',
...
#write_csv(missed_obis_sub, here(obisdata, paste0(datefield, 'obis_cti-filtered.csv')))
# if none missed
write_csv(obis_sub, here(obisdata, paste0(datefield, 'obis_cti-filtered.csv')))
```