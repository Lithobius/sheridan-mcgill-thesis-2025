...
NOTE pasted in from other repository but not tested yet 2025-12-12
...
# eDNA
edna_combined <- here('processeddata', 'peco')
...
obis_records <- read_csv(here(obisdata, '20250605_obis_cti-filtered.csv'))
```
...
```{r if intermediate}
obis_found <- obis_spacetime_bb %>%
  mutate(found = paste0(decimal_latitude, '-', decimal_longitude, '-', date_start))

obis_new <- obis_distinct_spacetime %>%
  mutate(found = paste0(decimal_latitude, '-', decimal_longitude, '-', date_start)) %>%
  filter(!(found %in% obis_found$found)) %>%
  select(!found)

# turn into coordinate points
obis_spacetime_sf_new <- st_as_sf(obis_new, 
                    coords = c('decimal_longitude', 'decimal_latitude'),
                    crs = "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0",
                    remove = FALSE)

obis_spacetime_bb2 <- st_intersection(obis_spacetime_sf_new, bbox_sf)

obis_spacetime_bb <- bind_rows(obis_spacetime_bb, obis_spacetime_bb2)
```
...
write_rds(obis_spacetime_bb,here(obisdata, '20250722_obis_spacetime_bb.rds'))
#obis_spacetime_bb <- read_rds(here(obisdata, '20250605_obis_spacetime_bb.rds'))
obis_spacetime_bb <- read_rds(here(obisdata, '20250722_obis_spacetime_bb.rds'))
...
# it errored out; write a loop for the error
...
# classify
some of this just makes more sense to do in python...


```{r stupid loop to extract bbox date filename}
...
# read in raw csvs
extract_day_raw <- read_csv(here(erd_data, '20250722_obis_erddap_to-extract_pathfinder-day.csv'),
...
# if needed
extract_all <- read_csv(here(erd_data, '20250731_pathfinder_extract_key.csv')) 
...
extract_day_raw2 <- read_csv(here(erd_data,
                                 '20250722_obis_erddap_to-extract_pathfinder-day-missed.csv'),
                            col_names = c('box_name', 'date_1', 'date_2', 'filename'))
extract_night_raw2 <- read_csv(here(erd_data,
                                   '20250722_obis_erddap_to-extract_pathfinder-night-missed.csv'),
                            col_names = c('box_name', 'date_1', 'date_2', 'filename')) %>%
  distinct()
...
extracted_sst <- read_csv(here(erd_data, '20250803_obis_erddap_extract_pathfinder-day.csv'),
...
extracted_sst_n <- read_csv(here(erd_data,
                                 '20250803_obis_erddap_extract_pathfinder-night.csv'),
                          col_names = c('id', 'sst_n', 'filename', 
                                        'layer',
                                       'year', 'month','box_name_n', 
                                        'decimal_latitude', 'decimal_longitude',
                                        'geometry')) %>%
  # don't need these anymore
  select(!c(id, geometry)) %>%
...
obis_matched <- obis_records %>%
  ...
  # year only, date only
  mutate(remove = case_when(str_detect(event_date, '^([0-9]{4})$') ~ 'x',
                            str_detect(event_date, '^([0-9]{2})$') ~ 'x',
                            n > 1 & event_date %in% c('2016-04-21T08:30:00Z') ~ 'x',
                            year == '1998' & event_date == '1996-06-26' ~ 'x',
                            year == '1985' & event_date == '1986-01-06' ~ 'x',
                            year == '2001' & event_date == '2002-01-22' ~ 'x',
                            year == '2003' & event_date == '2002-07-01' ~ 'x',
                            year == '2006' & event_date == '2007-01-04' ~ 'x',
                            year == '1996' & event_date == '1997-01-01' ~ 'x',
                            year == '1994' & event_date == '1995-01-01' ~ 'x',
                            event_date %in% c('06-01') ~ 'x',
                            # check these, multiple boxes for a record
                            n == 2 & event_date == '1989-05-20T02:00Z' & box_name_combine == 'newfoundland_2' ~ 'x',
...
# load in and give headers
extracted_sst_8000 <- read_csv(here(erd_data, '20250803_obis_erddap_extract_pathfinder-day_buff.csv'),
                          col_names = c('id', 'sst', 'filename',
                                         'layer',
                                        'year', 'month', 'box_name',
                                        'decimal_latitude', 'decimal_longitude',
...
extracted_sst_n_8000 <- read_csv(here(erd_data,
                                 '20250803_obis_erddap_extract_pathfinder-night_buff.csv'),
                          col_names = c('id', 'sst_n', 'filename', 
                                        'layer',
                                       'year', 'month', 'box_name_n', 
                                        'decimal_latitude', 'decimal_longitude',
...
# load in and give headers
extracted_sst_16000 <- read_csv(here(erd_data, '20250803_obis_erddap_extract_pathfinder-day_buff2.csv'),
                          col_names = c('id', 'sst', 'filename',
                                         'layer',
                                        'year', 'month','box_name',
                                        'decimal_latitude', 'decimal_longitude',
...
extracted_sst_n_16000 <- read_csv(here(erd_data,
                                 '20250803_obis_erddap_extract_pathfinder-night_buff2.csv'),
                          col_names = c('id', 'sst_n', 'filename', 
                                        'layer',
                                       'year', 'month', 'box_name_n',
                                        'decimal_latitude', 'decimal_longitude',
...
write_csv(should_have, here(obisdata, '20250725_should-have.csv'))
...
write_csv(bb_tomake, here(obisdata, '20250725_bb-to-make.csv'))
...
# lets just do a few alaska ones
#miss_shouldhave <- obis_spacetime_bb %>%
#  filter(box_name %in% c('puget_sound')) %>%
#    mutate(year = lubridate::year(date_start),
#         month = lubridate::month(date_start))
...
extracted_sst_shouldhave <- read_csv(here(erd_data,
                                          '20250515_obis_erddap_extract_pathfinder-day_shouldhave.csv'),
