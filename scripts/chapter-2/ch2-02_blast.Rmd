...
Changes Sept 2024:
We're greatly reducing our usage of ASVs with 12S MiFish moving forward so I've eliminated some of the unnecessary steps to add in ASV conversions. Instead we're just going to use asv-level assignments for all ASVs and as a result, only BLAST the cluster representative.
...
#                           '20251111_peco-combined_12s_post-lulu-asvs.fasta')
edna_to_blast <- file.path(fastq_path,
                           '20251111_peco-combined_12s_asv-sequences.fasta')
blast_out <- file.path(fastq_path, paste0(datefield_new, runfield, 'swarm_blastresults_asv.txt'))
...
```{r cleanup-asvs}
# top 10 list
asv_50blast <- read_delim(here(fastq_path, paste0(datefield_new, runfield, 'swarm_blastresults_asv.txt')),
                           delim = '\t', col_names = c("asv", "identity_percentage", 
...
# asv numbers to join with the blast results
asv_key <- read_csv(here(fastq_path, '20251111_peco-combined_12s_lulu_motu2asv.csv')) %>%
...
blast50_updatetaxa <- asv_50blast %>%
  ...
  select(asv, identity_percentage, evalue, found_taxa, common_name, rank, 
...
# Here we're going to assign to asvs from scratch
# First; assign all asvs with 100% one species
...
Now we'll use updated taxa from the conflicts, and add a column for asv level ids based on seed asv
```{r update}
# drop uncertanties to next taxonomic level
blast_updates <- blast_conflict %>%
      # modify dataframe to be more useful
  select(asv, found_taxa, updated_taxon, common_name,
  ...
  relocate(asv, found_taxa, common_name, rank) %>%
  arrange(asv) %>%
...
asv45: castor canadensis
asv61: homo sapiens
asv132: bacteria
asv871: ralstonia bacteria etc.

```{r assign new}
taxonomy_assigned_asv <- asv_key %>%
...
taxonomy_assigned_motu <- taxonomy_assigned_asv %>%
  ...
  select(!c(one_hit, one_com, sp_100, higher_taxa, fam_100, gen_100, sp_conflict,
            motu_swarm, curated, lulu_rank, low_match, very_low_match, extremely_low_match,
