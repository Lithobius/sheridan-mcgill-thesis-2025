...
"""
The data for 2021 came off the Nextseq with
...
LOG_FILENAME = 'rename2024-log.txt'
        ...
        if filename.endswith('gz'):
            ...
            try:
                ...
                try:
                    ...
                    peconum = bits[0] + bits[1] + bits[2] + bits[3] + bits[4]
                    #logging.info(peconum)
                    try:
                        #bits[0] = re.sub(peconum, name_dic[peconum], peconum)
                        bits[0] = re.sub(bits[0], "Neg", "")
                        bits[1] = re.sub(bits[1], "_", "")
                        bits[2] = re.sub(bits[2], "PCR[0-9]+", "")
                        bits[3] = re.sub(bits[3], "_", "")
                        bits[4] = re.sub(bits[4], "2024", "")
                        newbits = "".join(bits)
                        #logging.info(newbits)
...
if __name__ == "__main__":
    ...
    direct = here('./rawdata/peco/fastq/12S_PECO2024_20250525/Analysis/1/Data/fastq/')
    # destination directory for 'good' files
    dest_direct = here('./rawdata/peco/fastq/12S_PECO2024_20250525/Analysis/1/Data/fastq/fixed/')
    ## csv of replacements
    replacements = here('./rawdata/peco/2024_edna/peco_2024_name-replacement-nextseq.csv')
