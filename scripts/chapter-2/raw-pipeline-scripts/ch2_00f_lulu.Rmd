...
fastq <- here('rawdata', 'peco', 'fastq', 'allyears')
...
# load in 
```{r loadin}
motu2asv <- read_csv(here(fastq, '20250528_peco-combined_12s_motu2asv.csv'))%>%
  distinct()

asv_seq_table <- read_delim(here(fastq, 
                                 "20250528_peco-combined_12s_sequence-table_asv-names.txt"))
```
...
otu_sequences <- file.path(fastq,
                           '20250528_peco-combined_12s_swarm_clusterreps-motus.fasta')
