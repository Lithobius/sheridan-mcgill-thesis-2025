...
edna_out <- here('processeddata', 'peco', '2024_edna')

# fields
datefield <- '20250711_'
runfield <- 'peco-2024_12s_'
```
...
asvtable <- read_delim(here(fastq,
                            '20250528_peco-combined_12s_sequence-table_asv-names_2024.txt'))
...
motumatrix_metadata <- asvtable %>%
  ...
  filter(sample_id %in% edna_metadata_pcr$sample_id) %>%
...
write_csv(motumatrix_metadata_fish, here(edna_out,
                                         paste0(datefield, 'peco2024_12s-asvmatrix_fish-temp.csv')))


write_csv(motumatrix_metadata_nofield, here(edna_out, paste0(datefield, 'peco2024_12s-asvmatrix-temp.csv')))
...
asvmatrix_simple <- motumatrix_metadata_nofield %>%
  ...
  select(!c(#biological_observations,
            ...
            swell_m, 
            tide, wind, rain,
            seagrass_presence, seagrass_notes,
            bottom_depth_m, collection_method,
            #site_or_sampling_description, 
            organization_participants)) %>%
  #reads together
  relocate(in_ctrl_field, reads_raw, reads_adjust_lab, reads_adjust_field, .before = reads_adjust_fieldlab)


write_csv(asvmatrix_simple, here(edna_out, paste0(datefield, 'peco2024_12s-asvmatrix-simple.csv')))
...
write_csv(lab_controls_to_merge, here(edna_out, paste0(datefield, 'peco2024_12s_controls.csv')))
...
```{r}
testing_again <- motumatrix_metadata_fish %>%
  select(sample_id, asv, found_taxa, common_name, extraction_blank, in_ctrl_extract, reads_raw,
         pcr_blank, in_ctrl_pcr, region_name) %>%
  distinct()


testing <- motumatrix_metadata %>%
  filter(treatment == 'Extraction blank') %>%
  select(sample_id, pcr_blank) %>%
  distinct()
```
