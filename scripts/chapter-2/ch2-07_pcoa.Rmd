...
These are matched to MiFish species
...
obisgbif_mifish_clusters <- obisgbif_mifish_expldiss$matrix[,21] %>%
  as.data.frame()
...
#ggsave(filename = here(pcoa, '20230423_peco_pcoa-site.pdf'),
...
obisgbif_cons <- read.tree(here(pcoa_out, 
                                '20250927_obisgbif-mifish_clusters-site.tre'))
...
These are all fish found in our region.
...
unfilter_cons <- read.tree(here(pcoa_out, 
                                '20250927_obisgbif-unfilter_clusters-site.tre'))
#check for polytomies? true = good
...
# extracting group identities from matrix at chosen cut (column)
mifish_2021_clusters <- mifish_2021_expldiss$matrix[,13] %>%
  as.data.frame()
...
# extracting group identities from matrix at chosen cut (column)
mifish_2022_clusters <- mifish_2022_expldiss$matrix[,13] %>%
  as.data.frame()
...
# extracting group identities from matrix at chosen cut (column)
mifish_2023_clusters <- mifish_2023_expldiss$matrix[,13] %>%
  as.data.frame()
...
# extracting group identities from matrix at chosen cut (column)
mifish_2024_clusters <- mifish_2024_expldiss$matrix[,8] %>%
  as.data.frame()
...
ggsave(filename = here(pcoa_out, paste0(datefield,'regionyear-tree_all.pdf')),
       height = 22,
       width = 4,
       unit = 'in')
