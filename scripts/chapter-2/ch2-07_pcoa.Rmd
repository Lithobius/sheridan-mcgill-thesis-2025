...
# save in
pcoa_out <- here::here("output", "chapter2", "pcoa")
...
obisgbif <- read_csv(here(bigdata,
               '20251212_obis-gbif_region-peco-bysite_unfiltered.csv')) %>%
...
ape::write.tree(obisgbif_mifish_upgma$cons, 
                here(pcoa_out, paste0(datefield, 'obisgbif-mifish_clusters-site-year.tre')))
...
ggsave(filename = here(pcoa_out, paste0(datefield, 'obisgbif_pcoa-site-year.png')),
       device = 'png', dpi=300, height = 8, width= 13,  units = "in")

ggsave(filename = here(pcoa_out, paste0(datefield, 'obisgbif_pcoa-site-year.pdf')),
...
```{r obisgbif tree}
obisgbif_cons_y <- read.tree(here(pcoa_out, 
                                paste0(datefield,'obisgbif-mifish_clusters-site-year.tre')))
#check for polytomies? true = good
is.binary(obisgbif_cons_y)

#set up labels
obisgbif_tree_plot <- obisgbif_mifish_plot %>%
  mutate(tip.label = as.factor(region_label))

ggtree(obisgbif_cons_y,
...
# extracting group identities from matrix at chosen cut (column)
obisgbif_mifish_clusters <- obisgbif_mifish_expldiss$matrix[,10] %>%
  as.data.frame()
...
obisgbif_cons <- read.tree(here(pcoa_out, 
                                paste0(datefield, 'obisgbif-mifish_clusters-mifish.tre')))
#check for polytomies? true = good
...
# extracting group identities from matrix at chosen cut (column)
mifish_site_clusters <- mifish_site_expldiss$matrix[,11] %>%
  as.data.frame()
...
ggsave(filename = here(pcoa_out, paste0(datefield, 'peco_pcoa-site-sp.png')),
       device = 'png', dpi=300, height = 8, width= 13,  units = "in")

ggsave(filename = here(pcoa_out, paste0(datefield, 'peco_pcoa-site-sp.pdf')),
       device = 'pdf', dpi=300, height = 8, width= 8,  units = "in")
```

```{r}
# write tree
ape::write.tree(mifish_site_upgma$cons, here(pcoa_out, paste0(datefield, 'region-site_clusters-sp.tre')))

mifish_cons <- read.tree(here(pcoa_out, paste0(datefield, 'region-site_clusters-sp.tre')))
#check for polytomies? true = good
is.binary(mifish_cons)
...
ggsave(filename = here(pcoa_out, paste0(datefield,'region-tree_sp-site.pdf')),
       height = 11,
       width = 4,
       unit = 'in')
...
ggsave(filename = here(pcoa_out, paste0(datefield, 
                                    'peco_pc1xlat-site-motu_pa_boxplot-site_presentation.pdf')),
       device = 'pdf', height = 10, width= 10,  units = "in")
...
ggsave(filename = here(pcoa_out, paste0(datefield, 
                                    'obisgbif-mifish_pc1xlat_boxplot-site_presentation.pdf')),
       device = 'pdf', height = 10, width= 10,  units = "in")
...
ggsave(filename = here(pcoa_out, paste0(datefield,
                                    'obisgbif-unfiltered_pc1xlat_boxplot-site_presentation.pdf')),
       device = 'pdf', height = 10, width= 10,  units = "in")
```
