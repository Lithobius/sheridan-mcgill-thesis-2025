...
ggsave(filename =  here('output', 'maps', 
                                 paste0(datefield, 'map_edna_2021-24_centroids-blue.pdf')), 
       device = "pdf", dpi = 300, width = 8, height = 12, units = "in")

```

# Haida Gwaii

lon = c(-133.2, -130.5, -130.5, -133.2), lat = c(50.2, 50.2, 54.3, 54.3)))


```{r haida gwaii}
hg_land <- st_read(here('rawdata', 'spatial', 'hg.gpkg'), layer = 'land')

#convert to to lat long
hg_land_t <- st_transform(hg_land, "EPSG:4326")

# make a bounding box
hg_bbox <- st_bbox(c(xmin = -138, xmax = -130.5, ymin = 50.5, ymax = 54.3), 
                     crs = "EPSG:4326")

# st_crop to use the bounding box to crop the original object
hg_crop <- st_crop(hg_land_t, hg_bbox)

hg_map <- ggplot(hg_crop) +
  geom_sf(fill = 'white', 
          color = 'grey50') +
  xlim(min(-133.5, 
    na.rm = T), 
    max(-130.5, na.rm = T)) +
  ylim(min(51.8, na.rm = T), 
       max(54.3, na.rm = T)) + 
  theme_classic() +
    theme(axis.title = element_blank(),
        axis.text = element_blank(),
...
hg_map + geom_point(data = hg_subset,
    ...
    geom_text(data = hg_subset,
            aes(x = decimal_longitude +.1,
                 y = decimal_latitude +.1,
                label =site_code_plot))

ggsave(here('output', 'maps', paste0(datefield, 'haidagwaii.pdf')))
...
ggsave(here('output', 'maps', paste0(datefield, 'haidagwaiismall.pdf')),
       height = 3,
       width = 3,
       units = 'in')
...
ggsave(here('output', 'maps', paste0(datefield, 'haidagwaiismall-blue.pdf')),
       height = 3,
       width = 3,
       units = 'in')

ggsave(here('output', 'maps', paste0(datefield, 'haidagwaiismall-blue.png')),
       height = 3,
       width = 3,
       units = 'in')
