...
# here filepaths
splists <- here('processeddata', 'splists')
spatial <- here("rawdata", "spatial", "MEOW")
```
...
Here we're using the MEOW ecoregions to define the species pools.
...
# note when running this with renv I got an error here but it did not seem to affect the output
```
...
write_csv(nep_fish, here(splists, paste0(datefield, 'obis-fish_nep.csv')))
...
write_csv(bc_fish, here(splists, paste0(datefield, 'obis-fish_bc.csv')))
```
...
write.csv(obisfish_basefao_trait, here(splists, 
                                       paste0(datefield, "obis-fish_fishbase_bc.csv")))
...
# load-in keys
taxa_key <- read_csv(here(splists,
                     '20230929_fish_ncbi-worms_by-accession.csv'))[-1]

common_name <- read_csv(here(splists,
...
write.csv(full_splist, here(splists, 
                            paste0(datefield, "fishlist_obis-hakai.csv")))
```
