...
bc <- st_read(here("rawdata","spatial", "BCcoast", 
                   "NTS_BC_COASTLINE_POLYS_125M","250_CST_PY_polygon.shp")) %>% 
  st_make_valid() %>%
...
ggsave(here(map_out, '2018Calvert_site.png'), device = png,
       height = 7, width = 5, units = 'in')

ggsave(here(map_out, '2018Calvert_site.pdf'), device = pdf,
       height = 7, width = 5, units = 'in')
```
