...
sp_ranges <- edna_peco %>%
  select(found_taxa, taxa_mean) %>%
  distinct() %>%
  dplyr::rename(peco_taxa_mean = taxa_mean)

# set up regions
regions <- read_csv(here(dataedna, '20260320_peco_region-name-plotting.csv'))[-1] %>%
  dplyr::rename(region_label = region_name_label) 
...
only_2023 <- edna_compare %>%
  filter(year == 2023) %>%
  filter(rank == "species") %>%
  mutate(index_type = case_when(library == '2023first' ~ 'Combinatorial dual indexing',
                                library == '2023redone' ~ 'Unique dual indexing'))
