...
datefield <- '20260404_'
datefield_new <- '20260404_'
# if different
datefield_swarm <- '20260404_'
...
edna_to_blast <- file.path(pecofastq,
                           '20260404_peco_next-mi-combined_asv-sequences.fasta')
...
motu_key <- read_csv(here(pecofastq, '20260404_peco_next-mi-combined_lulu_motu2asv.csv')) %>%
...
taxonomy_assigned_asv <- motu_key %>%
 ...
 # left_join(common_name) %>%
  ...
  mutate(found_taxa = case_when(found_taxa == 'Homo' ~ 'Homo sapiens',
                                TRUE ~ found_taxa)) %>%
  mutate(rank = case_when(class %in% c('Mammalia', 'Aves', 'Insecta') ~ 'outgroup',
