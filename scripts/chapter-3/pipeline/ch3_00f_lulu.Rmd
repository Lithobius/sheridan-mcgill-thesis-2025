...
# load in 
```{r loadin}
motu2asv <- read_csv(here(fastq, '20260404_peco_next-mi-combined_motu2asv.csv'))%>%
  distinct()

asv_seq_table <- read_delim(here(fastq, 
                                 "20260404_peco_next-mi-combined_sequence-table_asv-names.txt"))
```
...
otu_sequences <- file.path(fastq,
                           '20260404_peco_next-mi-combined_swarm_clusterreps.fasta')
